...
class analyzerMainWindow(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        self.filterLabel1 = QtGui.QLabel('running average:')
        ...
        self.filterLabel2 = QtGui.QLabel('running average:')
    ...
    def keyPressEvent(self, e):
        ...
        # arrow keys increase running average length:
        # up/down, increase/decrease core size 1
        # right/left, increase/decrease core size 2
        if e.key() == QtCore.Qt.Key_Up:
            self.filterCoreSize1 = int(float(self.filterCoreSizeBox1.text()))+2
        ...
        if e.key() == QtCore.Qt.Key_Right:
            self.filterCoreSize2 = int(float(self.filterCoreSizeBox2.text()))+2
            self.filterCoreSizeBox2.setText('%d' % self.filterCoreSize2) 
            self.analyzePeak2()       
            
        if e.key() == QtCore.Qt.Key_Left:
            if self.filterCoreSize2 > 1:                
                self.filterCoreSize2 = int(float(self.filterCoreSizeBox2.text()))-2
                self.filterCoreSizeBox2.setText('%d' % self.filterCoreSize2)
                self.analyzePeak2()
    ...
    def movingAverage1(self):
        if self.section1.__len__() >= self.filterCoreSize1:
            #self.sectionAve = np.zeros(self.section1.__len__())
            self.sectionAve = list(self.section1)
            #print(np.amax(self.sectionAve))
            #self.sectionAve = self.section1
            for m in range(int((self.filterCoreSize1-1)/2), int(self.section1.__len__()-(self.filterCoreSize1-1)/2)):
                sum = 0.0
                for n in range(int(m-(self.filterCoreSize1-1)/2),int(m+(self.filterCoreSize1-1)/2)+1):
                    sum = sum+self.section1[n]
                self.sectionAve[m]=sum/self.filterCoreSize1
            self.section1 = list(self.sectionAve)
        else:
            print ('Section length too small for filtering.')
                 
    def movingAverage2(self):
        if self.section2.__len__() >= self.filterCoreSize2:
            #self.sectionAve = np.zeros(self.section1.__len__())
            self.sectionAve = list(self.section2)
            #print(np.amax(self.sectionAve))
            #self.sectionAve = self.section1
            for m in range(int((self.filterCoreSize2-1)/2), int(self.section2.__len__()-(self.filterCoreSize2-1)/2)):
                sum = 0.0
                for n in range(int(m-(self.filterCoreSize2-1)/2),int(m+(self.filterCoreSize2-1)/2)+1):
                    sum = sum+self.section2[n]
                self.sectionAve[m]=sum/self.filterCoreSize2
            self.section2 = list(self.sectionAve)
        else:
            print ('Section length too small for filtering.')
